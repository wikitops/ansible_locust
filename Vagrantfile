...
$instance_name_prefix   = "locust"
$vm_memory              = 2048
$vm_cpus                = 1
$subnet                 = "10.0.0.4" # For 10.0.0.4X
    ...
    config.vm.define vm_name = "%s%02d" % [$instance_name_prefix, i] do |server|
      ...
      if i == $num_instances
        config.vm.provision "ansible" do |ansible|
          ...
          ansible.groups = {
            "locust" => ["#{$instance_name_prefix}0[1:#{$num_instances}]"],
            "locust-master" => ["#{$instance_name_prefix}0[1:#{$num_instances_master}]"],
            "locust-slave" => ["#{$instance_name_prefix}0[2:#{$num_instances}]"]
          }
        end
      end
    end
